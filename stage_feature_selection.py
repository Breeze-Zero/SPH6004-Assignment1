...
# X_train, y_train = smote.fit_resample(X_train, y_train["aki_stage"])
...
toolbox.register("evaluate", evaluate, X=scaler.fit_transform(X_train), y=y_train.values.ravel())

# 设置并行计算（如果需要）
def parallel_eval(individual):
    return evaluate(individual, X=scaler.fit_transform(X_train), y=y_train.values.ravel())
